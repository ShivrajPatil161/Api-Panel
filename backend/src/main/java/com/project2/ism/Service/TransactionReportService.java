...
public class TransactionReportService {
    ...
    /**
     * Generate merchant transaction report with details and summary
     */
    public TransactionReportResponse generateMerchantTransactionReport(TransactionReportRequest request) {
        logger.info("Generating merchant transaction report for date range: {} to {}",
        ...
        try {
            ...
            Page<MerchantTransactionDetails> transactionPage = merchantTransactionRepository
            ...
            // Get summary data
            TransactionSummary summary = getMerchantTransactionSummary(request);
            ...
            logger.info("Successfully generated merchant transaction report with {} transactions",
                    transactionDetails.size());
            return response;

        } catch (Exception e) {
            logger.error("Error generating merchant transaction report", e);
            throw new BusinessException("Failed to generate merchant transaction report: " + e.getMessage());
        }
    }
     ...
     */
    public TransactionReportResponse generateFranchiseTransactionReport(TransactionReportRequest request) {
        logger.info("Generating franchise transaction report for date range: {} to {}",
                request.getStartDate(), request.getEndDate());
        ...
        try {
            ...
            // Fetch transaction details
            Page<FranchiseTransactionDetails> transactionPage = franchiseTransactionRepository
            ...
            FranchiseTransactionSummary summary = getFranchiseTransactionSummary(request);
            ...
            response.setSummary(summary);
            response.setReportGeneratedAt(LocalDateTime.now());
            response.setReportType("FRANCHISE_TRANSACTION_REPORT");
            response.setTotalPages(transactionPage.getTotalPages());
            response.setTotalElements(transactionPage.getTotalElements());
            response.setHasNext(transactionPage.hasNext());
            response.setHasPrevious(transactionPage.hasPrevious());

            logger.info("Successfully generated franchise transaction report with {} transactions",
                    transactionDetails.size());
            return response;

        } catch (Exception e) {
            logger.error("Error generating franchise transaction report", e);
            throw new BusinessException("Failed to generate franchise transaction report: " + e.getMessage());
        }
    }

    /**
     * Get merchant transaction summary only
     */
    public TransactionSummary getMerchantTransactionSummary(TransactionReportRequest request) {
        validateReportRequest(request);

        Map<String, Object> summaryData = merchantTransactionRepository.getMerchantTransactionSummary(
                request.getStartDate(),
                request.getEndDate(),
                request.getMerchantId(),
                request.getTransactionStatus(),
    ...
    /**
     * Get franchise transaction summary with commission breakdown
     */
    public FranchiseTransactionSummary getFranchiseTransactionSummary(TransactionReportRequest request) {
        validateReportRequest(request);
     ...
     */
    public List<Map<String, Object>> getFranchiseMerchantPerformance(TransactionReportRequest request) {
        validateReportRequest(request);

        List<Object[]> performanceData = franchiseTransactionRepository.getFranchiseMerchantPerformance(
                request.getStartDate(),
                request.getEndDate(),
                request.getFranchiseId());
