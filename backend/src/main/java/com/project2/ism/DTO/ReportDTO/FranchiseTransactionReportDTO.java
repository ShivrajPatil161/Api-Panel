...
public class FranchiseTransactionReportDTO {
    ...
    public FranchiseTransactionReportDTO(
            ...
            String authCode,
            String tid,
            BigDecimal merchantNetAmount,   // mtd.netAmount
            BigDecimal grossCharge,         // mtd.grossCharge (systemFee + franchiseCommission)
            BigDecimal franchiseCommission, // ftd.netAmount
            BigDecimal systemFee,           // mtd.charge
            String brandType,
            String cardType,
        ...
        if (txnAmount != null && txnAmount.compareTo(BigDecimal.ZERO) > 0) {
            // Merchant rate = what merchant pays (txnAmount - settleAmount)
            this.merchantRate = txnAmount
                    .subtract(settleAmount)
                    .divide(txnAmount, 4, RoundingMode.HALF_UP)
                    .multiply(BigDecimal.valueOf(100));

            // Franchise rate = what franchise gets (txnAmount - (settleAmount + franchiseCommission))
            this.franchiseRate = txnAmount
                    .subtract(settleAmount.add(commissionAmount))
                    .divide(txnAmount, 4, RoundingMode.HALF_UP)
                    .multiply(BigDecimal.valueOf(100));

            // Commission rate = merchantRate - franchiseRate (system's share)
            this.commissionRate = merchantRate.subtract(franchiseRate);

            // Settlement percentage
            this.settlementRate = txnAmount
                    .subtract(settleAmount)
                    .divide(txnAmount, 4, RoundingMode.HALF_UP)
                    .multiply(BigDecimal.valueOf(100));
        } else {
            this.settlementRate = BigDecimal.ZERO;
